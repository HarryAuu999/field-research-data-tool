...
| 图片上自由涂画范围 | 支持 | 使用`imageRange`，记录归一化笔画；底图必须内嵌或随应用缓存；可按明确配置显示人数热力图，但尚无路径约束 |
...
参见 [耳挂耳机接触与疼痛范围记录](../examples/ear-image-range-test.json)。此题型用于直接在底图上自由涂画范围，点按也可记录很小的范围；鼠标或手指笔画以相对原图宽高的归一化坐标（0～1）记录，重绘不提高同等级颜色强度。它必须独占一页，不能设置`page`。可撤销本页最后一笔；确认“清除所有标记”后会清除本页全部等级，且不能用撤销恢复。
...
填写页按上一题／下一题切换，每张底图独占一页。画笔滑块设定之后新笔画的宽度，宽度随每笔保存；正式版 V1.4.0 在笔画结束时按图片宽度的0.3%固定容差简化坐标，界面不提供容差设置。撤销只移除当前页的最后一笔；“清除所有标记”经确认后清除当前页全部等级，不影响其他图片页。已有未简化笔画无需迁移，仍可显示和分析。
