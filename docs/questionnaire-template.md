...
Beta填写页仍按上一题／下一题切换，每张底图独占一页。画笔滑块设定之后新笔画的宽度，宽度随每笔保存；笔画结束时按图片宽度的0.15%固定容差简化坐标，界面不提供容差设置。内置侧面与截面耳图在标注页采用紧凑视框，完整显示耳朵并沿用原图坐标；背面图保持原样。撤销只移除当前页的最后一笔；“清除所有标记”经确认后清除当前页全部等级，不影响其他图片页。已有未简化笔画无需迁移，仍可显示和分析。
